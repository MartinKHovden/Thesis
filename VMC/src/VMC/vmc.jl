...
function runVMC!(system,
    ...
    for k = 1:numOptimizationIterations
        ...
        # println(system.particles, reverse(system.particles,dims =1))
        # println("symmetry = ", (computePsi!(system.wavefunctionElements[3], system.particles )* computePsi!(system.wavefunctionElements[3], reverse(system.particles,dims=1)))/(computePsi!(system.wavefunctionElements[3], system.particles )* computePsi!(system.wavefunctionElements[3], system.particles)))
        # push!(symmetry, (computePsi!(system.wavefunctionElements[3], system.particles )* computePsi!(system.wavefunctionElements[3], reverse(system.particles,dims=1)))/(computePsi!(system.wavefunctionElements[3], system.particles )* computePsi!(system.wavefunctionElements[3], system.particles)))
    end
    ...
    if writeToFile
        ...
        saveDataToFile(symmetry, "symmetryTest.txt")
    end
