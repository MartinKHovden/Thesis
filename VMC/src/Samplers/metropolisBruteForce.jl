...
function metropolisStepBruteForce(stepLength, system::gaussianJastrowNNAnalytical)
    numParticles = system.numParticles 
    numDimensions = system.numDimensions
...
function computeDriftForce(system::slaterJastrowNNAnalytical, particleNumber::Int64, dimension::Int64)
    return slaterDeterminantComputeDriftForce(system, particleNumber, dimension) +
            + slaterGaussianComputeDriftForce(system, particleNumber, dimension) +
            + jastrowComputeDriftForce(system, particleNumber, dimension) +
            + nnAnalyticalComputeDriftForce(system, particleNumber, dimension)
...
function computeRatio(system::gaussianJastrowNNAnalytical, particleToUpdate, coordinateToUpdate, oldPosition)
    ratioSlaterGaussian = slaterGaussianComputeRatio(system, oldPosition, particleToUpdate, coordinateToUpdate)
    ratioNNAnalytical = nnAnalyticalComputeRatio!(system, oldPosition)
    ratioJastrow = simpleJastrowComputeRatio(system, oldPosition, particleToUpdate)
    println(ratioSlaterGaussian," ", ratioNNAnalytical," ", ratioJastrow)
    return ratioSlaterGaussian*ratioNNAnalytical*ratioJastrow
end
...
function computeRatio(system::slaterJastrowNNAnalytical, particleToUpdate, coordinateToUpdate, oldPosition)
    R = slaterMatrixComputeRatio(system, particleToUpdate)
    ratioSlaterDeterminant = R^2
    ratioSlaterGaussian = slaterGaussianComputeRatio(system, oldPosition, particleToUpdate, coordinateToUpdate)
    ratioNNAnalytical = nnAnalyticalComputeRatio!(system, oldPosition)
    ratioJastrow = jastrowComputeRatio(system, oldPosition, particleToUpdate)
    return ratioSlaterDeterminant*ratioSlaterGaussian*ratioNNAnalytical*ratioJastrow, R
end
"""
...
function runMetropolisBruteForce(system::slaterNNAnalytical, num_mc_iterations, step_length; sampler = "bf", writeToFile=false, burn_in = 0.01)
    local_energy_sum::Float64 = 0.0
    ...
    if writeToFile
        ...
        filename = "../Data/MC/"*folder*"/nnAnalytical_"*sampler*"_step_length_" * string(step_length) *"_nodes_layer1_"* string(size(system.nn.w2)[2])* "_layer2_"*string(size(system.nn.w2)[1])  * "_num_p_" *string(system.numParticles)*"_num_d_" * string(system.numDimensions)* ".txt"  
        saveDataToFile(local_energies, filename)
    end
...
function runVMC(system::slaterNNAnalytical, numVMCIterations::Int64, numMonteCarloIterations, mc_step_length, optimizer; sampler = "bf", writeToFile=false)
    local_energies::Array{Float64, 2} = zeros(Float64, (numVMCIterations, 1))
    for k = 1:numVMCIterations
        local_energy, grads = runMetropolisBruteForce(system, numMonteCarloIterations, mc_step_length, sampler = sampler)
        # println(grads)
...
function runMetropolisBruteForce(system::slaterJastrowNNAnalytical, num_mc_iterations, step_length; burn_in = 0.01)
    local_energy_sum::Float64 = 0.0
    ...
    local_energy_psi_jastrow_param_derivative_sum = zeros(size(system.jastrowFactor.kappa))

    psi_jastrow_param_derivative_sum = zeros(size(system.jastrowFactor.kappa))
    ...
    for i = 1:num_mc_iterations
        ...
        psi_param_derivative = nnAnalyticalComputeParameterGradient!(system)
        psi_jastrow_param_derivative = jastrowComputeParameterGradient(system)
        ...
        if i > burn_in*num_mc_iterations
            ...
            local_energy_psi_jastrow_param_derivative_sum += local_energy*psi_jastrow_param_derivative
            ...
            psi_jastrow_param_derivative_sum += psi_jastrow_param_derivative
        end
    end
    ...
    mc_local_energy_psi_derivative_jastrow = local_energy_psi_jastrow_param_derivative_sum/samples
    ...
    mc_psi_derivative_jastrow = psi_jastrow_param_derivative_sum/samples
    ...
    local_energy_derivative_jastrow = 2*(mc_local_energy_psi_derivative_jastrow - mc_local_energy*mc_psi_derivative_jastrow)

    println(mc_local_energy)

    return mc_local_energy, [Array(local_energy_derivative_w1), Array(local_energy_derivative_w2), Array(local_energy_derivative_w3), Array(local_energy_derivative_b1), Array(local_energy_derivative_b2), Array(local_energy_derivative_b3), local_energy_derivative_jastrow]
    
end

function optimizationStep(system::slaterJastrowNNAnalytical, optimizer, grads)
    model = system.nn
    jastrow = system.jastrowFactor
    # println(jastrow.kappa)
    # println(jastrow.distanceMatrix)
    ...
    update!(optimizer, jastrow.kappa, grads[7])
end 
################################################################################
################################################################################
################################################################################
...
function runMetropolis(system::slaterRBM, num_mc_iterations::Int64, step_length::Float64; sampler = "bf", burn_in = 0.01, writeToFile = false)
    ...
    for i = 1:num_mc_iterations
        ...
        # println(psi_derivative_a, psi_derivative_b, psi_derivative_w)
    ...
    if writeToFile
        ...
        filename = "../Data/MC/"*folder*"/rbm_"*sampler*"_step_length_" * string(step_length) *"_num_hidden_"* string(length(system.nqs.h))* "_num_p_" *string(system.numParticles)*"_num_d_" * string(system.numDimensions)* ".txt"  
        saveDataToFile(local_energies, filename)
    end
...
function runVMC(system::slaterRBM, numVMCIterations::Int64, numMonteCarloIterations, mc_step_length, optimizer; sampler = "bf", writeToFile = false)
    ...
    if writeToFile
        ...
        filename = "../Data/Optimization/"*folder*"/rbm_"*sampler* "_mc_step_length"* string(mc_step_length) * "_learning_rate_" * string(optimizer.eta) * "_num_p_" *string(system.numParticles)*"_num_d_" * string(system.numDimensions)* ".txt"  
        saveDataToFile(local_energies, filename)
    end
...
function runMetropolisBruteForce(system::slaterJastrow, num_mc_iterations, step_length; sampler="bf", burn_in = 0.01, writeToFile= false)
    local_energy_sum::Float64 = 0.0
    ...
    if writeToFile
        ...
        filename = "../Data/MC/"*folder*"/jastrow_"*sampler*"_step_length_" * string(step_length) * "_num_p_" *string(system.numParticles)*"_num_d_" * string(system.numDimensions)* ".txt"  
        saveDataToFile(local_energies, filename)
    end
...
"""
function runVMC(system::slaterJastrow, numVMCIterations::Int64, numMonteCarloIterations, mc_step_length, optimizer; sampler="bf", writeToFile = false)
    local_energies::Array{Float64, 2} = zeros(Float64, (numVMCIterations, 1))
    for k = 1:numVMCIterations
        local_energy, grads = runMetropolisBruteForce(system, numMonteCarloIterations, mc_step_length, sampler=sampler)
        optimizationStep(system, optimizer, grads)
    ...
    if writeToFile
        ...
        filename = "../Data/Optimization/"*folder*"/jastrow_"*sampler*"_learning_rate_" * string(optimizer.eta) * "_num_p_" *string(system.numParticles)*"_num_d_" * string(system.numDimensions)* ".txt"  
        saveDataToFile(local_energies, filename)
    end
    return local_energies
end
...
function runMetropolisBruteForce(system::gaussianJastrowNNAnalytical, num_mc_iterations, step_length, iteration; burn_in = 0.01)
    local_energy_sum::Float64 = 0.0
...
function optimizationStep(system::gaussianJastrowNNAnalytical, optimizer, grads)
    model = system.nn
...
function runVMC(system::gaussianJastrowNNAnalytical, numVMCIterations::Int64, numMonteCarloIterations, mc_step_length, optimizer)
    println("test simple Jas gaussian")
    local_energies::Array{Float64, 2} = zeros(Float64, (numVMCIterations, 1))
    for k = 1:numVMCIterations
        local_energy, grads = runMetropolisBruteForce(system, numMonteCarloIterations, mc_step_length, k)
